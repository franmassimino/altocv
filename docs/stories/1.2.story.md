...
- [ ] **Task 1: Create Neon PostgreSQL Database** (AC: 1)
  - [ ] Sign up for Neon account at https://neon.tech
  - [ ] Create new project named "altocv2"
  - [ ] Create database within project (use default settings)
  - [ ] Copy connection string from Neon dashboard
  - [ ] Add `DATABASE_URL` to `.env.local` file
  - [ ] Verify `.env.local` is in `.gitignore`
  - [ ] Update `.env.example` with placeholder: `DATABASE_URL=postgresql://user:pass@host/db`

- [ ] **Task 2: Install and Initialize Prisma** (AC: 2)
  - [ ] Install Prisma CLI and Client: `pnpm add -D prisma` and `pnpm add @prisma/client`
  - [ ] Initialize Prisma: `npx prisma init`
  - [ ] Verify `prisma/schema.prisma` file was created
  - [ ] Verify `prisma/` directory exists in project root
  - [ ] Configure datasource to use PostgreSQL with `DATABASE_URL` from env

- [ ] **Task 3: Define Initial Database Schema** (AC: 3)
  - [ ] Define `User` model with all required fields per Data Models spec
  - [ ] Define `CV` model with all required fields per Data Models spec
  - [ ] Define `CreditTransaction` model with all required fields per Data Models spec
  - [ ] Add `UserTier` enum (FREE, PRO)
  - [ ] Add `SubscriptionStatus` enum (active, canceled, past_due, trialing)
  - [ ] Add `CreditTransactionType` enum with all transaction types
  - [ ] Define relationships: User -> CVs (1:N), User -> CreditTransactions (1:N)
  - [ ] Add necessary indexes per Database Schema spec
  - [ ] Verify schema follows Prisma best practices (cuid() for IDs, DateTime fields, cascading deletes)

- [ ] **Task 4: Create Database Client Utility** (AC: 4)
  - [ ] Create `/lib/db.ts` file
  - [ ] Implement Prisma Client singleton pattern for Next.js (prevents multiple instances in dev)
  - [ ] Export `db` instance for use across the application
  - [ ] Add TypeScript types from Prisma Client
  - [ ] Verify file follows coding standards (kebab-case filename)

- [ ] **Task 5: Generate Prisma Client and Run First Migration** (AC: 5)
  - [ ] Generate Prisma Client: `npx prisma generate`
  - [ ] Create first migration: `npx prisma migrate dev --name init`
  - [ ] Verify migration file created in `prisma/migrations/`
  - [ ] Verify migration applied successfully to Neon database
  - [ ] Check Neon dashboard to confirm tables exist

- [ ] **Task 6: Create and Run Database Test Script** (AC: 6)
  - [ ] Create test script: `scripts/test-db.ts` (or similar location)
  - [ ] Implement CRUD operations test:
    - Create a test user
    - Read the user back
  ...
  - [ ] Add script command to `package.json`: `"test:db": "tsx scripts/test-db.ts"`
  - [ ] Run test script and verify all operations succeed
  - [ ] Document test results in story completion notes

- [ ] **Task 7: Update Project Documentation** (AC: 6)
  - [ ] Update `.env.example` with complete database configuration
  - [ ] Add database setup instructions to README (or create docs/setup.md)
  - [ ] Document Prisma commands (generate, migrate, studio)
  - [ ] Document how to access Prisma Studio: `npx prisma studio`

- [ ] **Task 8: Write Unit Tests for Database Client** (Testing)
  - [ ] Create test file: `__tests__/lib/db.test.ts`
  - [ ] Test that db client exports successfully
  - [ ] Test that Prisma Client types are available
  - [ ] Mock Prisma Client for unit tests (use jest-mock-extended or similar)
  - [ ] Verify tests run successfully: `pnpm test`
