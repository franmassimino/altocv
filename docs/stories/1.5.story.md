...
- [ ] **Task 3: Configure Environment Variables in Vercel** (AC: 2)
  ...
  - [ ] Authentication:
    - [ ] `NEXTAUTH_SECRET` - Generate new secret for production (use `openssl rand -base64 32`)
    - [ ] `NEXTAUTH_URL` - Set to production domain (e.g., `https://altocv.vercel.app`)
...
- Environment variables required: `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`
...
NEXTAUTH_URL=https://altocv.vercel.app
...
**Important Notes:**
- `NEXTAUTH_SECRET`: Must be different for production (generate fresh secret)
- `NEXTAUTH_URL`: Must match production domain (update when custom domain is added)
