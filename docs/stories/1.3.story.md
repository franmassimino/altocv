...
- [x] **Task 1: Install and Configure NextAuth v5** (AC: 1, 3)
  ...
  - [x] Add `NEXTAUTH_SECRET` and `NEXTAUTH_URL` to `.env.example`
  - [x] Generate secure `NEXTAUTH_SECRET` for `.env.local`
...
- `.env.example` - Added NEXTAUTH_SECRET, NEXTAUTH_URL, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET
